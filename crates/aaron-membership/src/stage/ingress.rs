use aaron_core::{BoxError, CancellationToken, EventHub, QuicManager, read_frame, write_frame};
        ...
        match msg {
            ...
            Message::ConfigUpdate {
                ...
                if !tracing_filter.is_empty() {
                    let event = aaron_tracing::ChangeLogLevel::new(&tracing_filter);
                ...
                if !env_key.is_empty() {
                    let event = aaron_core::SetEnvVar {
                        key: env_key,
                        value: env_val,
                    };
                    self.event_hub.publish(event).await;
