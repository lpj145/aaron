use aaron_membership::{MembershipConfig, MembershipService, UpdateSwimConfig};
use aaron_core::{Context, Env, EventHub, Network, NodeId, Service, Store, Uuid};
use std::sync::Arc;
use std::time::Duration;
use tempfile::tempdir;
use tokio_util::sync::CancellationToken;
use aaron_tracing::ChangeLogLevel;
...
async fn test_cluster_config_propagation_over_quic() {
    ...
    let mut node2_env_events = ctx2.event_hub.subscribe::<aaron_core::SetEnvVar>().await;
