...
async fn test_raft_native_group_assigned_and_role_transitions() {
    ...
    event_hub
        .publish(ShardEvent::Bootstrap {
            shards: vec![
                aaron_core::ShardGroup {
                    shard_id: 1,
                    members: members.clone(),
                    role: aaron_core::MemberRole::Learner,
                },
                aaron_core::ShardGroup {
                    shard_id: 2,
                    members: members.clone(),
                    role: aaron_core::MemberRole::Voter,
                },
            ],
        })
        .await;
    ...
    match event {
        ShardEvent::Bootstrap { shards } => {
            ...
            assert_eq!(shards[0].role, aaron_core::MemberRole::Learner);
            assert_eq!(shards[1].shard_id, 2);
            assert_eq!(shards[1].role, aaron_core::MemberRole::Voter);
        }
    ...
    match event {
        ShardEvent::Join {
            ...
            assert_eq!(sid, 7);
            assert_eq!(m.len(), 3);
            assert_eq!(role, aaron_core::MemberRole::Voter);
        }
    ...
    event_hub
        .publish(ShardEvent::RoleChanged {
            shard_id,
            role: aaron_core::MemberRole::Leader,
        })
        .await;
    ...
    match event {
        ShardEvent::RoleChanged {
            ...
            assert_eq!(sid, 7);
            assert_eq!(role, aaron_core::MemberRole::Leader);
        }
