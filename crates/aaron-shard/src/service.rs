...
impl ShardService {
    ...
    pub fn coordinator(control_plane: ControlPlaneHandle) -> (Self, ShardHandle) {
        let nil_uuid = aaron_core::Uuid::NIL;
                                        ...
                                        loop {
                                            tokio::select! {
                                                ...
                                                stream_res = conn.accept_bi() => {
                                                    ...
                                                    tokio::spawn(async move {
                                                        while let Ok(Some(frame)) = aaron_core::read_frame(&mut recv).await {
                                                            if let Ok(aaron_control_plane::RaftMessage::ShardCommand {
                                                                shard_id,
                                                                role: _,
                                                                primary_high,
                                                                primary_low,
                                                                ...
                                                                op_type,
                                                                target_role,
                                                            }) = aaron_control_plane::RaftMessage::from_bytes(&frame) {
                                                                let primary = aaron_core::Uuid::new(primary_high, primary_low);
                                                                let replica_uuids: Vec<aaron_core::Uuid> = replicas
                                                                    .into_iter()
                                                                    .map(|(h, l)| aaron_core::Uuid::new(h, l))
                                                                    .collect();

                                                                let member_role = match target_role {
                                                                    0 => aaron_core::MemberRole::Learner,
                                                                    1 => aaron_core::MemberRole::Voter,
                                                                    _ => aaron_core::MemberRole::Leader,
                                                                };

                                                                match op_type {
                                                                    1 => {
                                                                        hub.publish(aaron_core::ShardEvent::RoleChanged {
                                                                            shard_id,
                                                                            role: member_role,
                                                                        }).await;
                                                                    }
                                                                    2 => {
                                                                        hub.publish(aaron_core::ShardEvent::Leave {
                                                                            shard_id,
                                                                        }).await;
                                                                    }
                                                                    _ => {
                                                                        ...
                                                                        hub.publish(aaron_core::ShardEvent::Join {
                                                                            shard_id,
                                                                            members: all_members,
                                                                            role: member_role,
                                                                        }).await;
                                                                ...
                                                                let resp = aaron_control_plane::RaftMessage::ShardCommandResp {
                                                                    success: true,
                                                                    shard_id,
                                                                ...
                                                                let _ = aaron_core::write_frame(&mut send, &resp.to_bytes()).await;
                                                            }
                                                        }
                                                    });
                                                }
                    ...
                    loop {
                        tokio::select! {
                            ...
                            _ = interval.tick() => {
                                ...
                                let msg = aaron_control_plane::RaftMessage::TelemetryHeartbeat {
                                ...
                                if let Ok(conn) = quic_worker.connect_node(&cp_addr, my_id_worker).await {
                                    if let Ok((mut send, _recv)) = conn.open_bi().await {
                                        let _ = aaron_core::write_frame(&mut send, &bytes).await;
                                        let _ = send.finish();
                                    }
                                }
                            }
...
fn spawn_event_hub_sync(
    ...
    event_hub: aaron_core::EventHub,
    handle: ShardHandle,
) {
    tokio::spawn(async move {
        let mut shard_events = event_hub.subscribe::<aaron_core::ShardEvent>().await;
        loop {
            tokio::select! {
                _ = token.cancelled() => break,
                event_res = shard_events.recv() => {
                    match event_res {
                        Ok(aaron_core::ShardEvent::Bootstrap { shards }) => {
                            let my_id = handle.local_node_id().await;
                            for g in shards {
                                let (primary, replicas) = if g.members.is_empty() {
                                    (aaron_core::Uuid::NIL, Vec::new())
                                } else if g.role == aaron_core::MemberRole::Leader {
                        ...
                        Ok(aaron_core::ShardEvent::Join { shard_id, members, role }) => {
                            let my_id = handle.local_node_id().await;
                            let (primary, replicas) = if members.is_empty() {
                                (aaron_core::Uuid::NIL, Vec::new())
                            } else if role == aaron_core::MemberRole::Leader {
                        ...
                        Ok(aaron_core::ShardEvent::RoleChanged { shard_id, role }) => {
                            handle.announce_role(shard_id, role).await;
                        }
                        Ok(aaron_core::ShardEvent::Leave { .. }) => {}
                        Err(_) => break,
                    }
                }
            }
