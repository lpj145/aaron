...
use aaron_membership::{Member, MemberStatus};
use aaron_core::Uuid;
...
impl MemberInfoResponse {
    ...
    fn from_member_with_rtt_and_raft(
        ...
        live_telemetry: Option<&aaron_control_plane::NodeTelemetrySnapshot>,
        local_telemetry: Option<&aaron_core::NodeTelemetry>,
    ) -> Self {
...
pub async fn leave_cluster(
        ...
        && let Some(mut local) = handle.local_member().await
    {
        ...
        state
            .ctx
            .event_hub
            .publish(aaron_membership::MembershipEvent::Left(local))
            .await;
    }
...
pub async fn start_node(
    ...
    let uuid = if let Some(ref nid) = payload.node_id
        && !nid.trim().is_empty()
    {
        nid.trim().parse::<aaron_core::Uuid>().map_err(|e| {
            (
                StatusCode::BAD_REQUEST,
    ...
    state
        .ctx
        .event_hub
        .publish(aaron_core::NodeEvent::StartNode {
...
pub async fn remove_node(
    ...
    let uuid = payload.node_id.trim().parse::<aaron_core::Uuid>().map_err(|e| {
        (
            StatusCode::BAD_REQUEST,
    ...
    state
        .ctx
        .event_hub
        .publish(aaron_core::NodeEvent::RemoveNode { node_id: uuid })
        .await;
