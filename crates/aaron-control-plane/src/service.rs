...
use aaron_core::{read_frame_with_limit, write_frame_with_limit, DEFAULT_MAX_RAFT_FRAME_SIZE, BoxError, Context, Service, ServiceConfig};
                                        ...
                                        let rpc_task = async {
                                            ...
                                            let resp_msg = match parsed {
                                                ...
                                                RaftMessage::ShardCommand {
                                                    ...
                                                    match op_type {
                                                        1 => {
                                                            event_hub.publish(aaron_core::ShardEvent::RoleChanged {
                                                                shard_id,
                                                                role: member_role,
                                                            }).await;
                                                        }
                                                        2 => {
                                                            event_hub.publish(aaron_core::ShardEvent::Leave {
                                                                shard_id,
                                                            }).await;
                                                        }
                                                        _ => {
                                                            ...
                                                            members.extend(&replica_uuids);
                                                             event_hub.publish(aaron_core::ShardEvent::Join {
                                                                shard_id,
                                                                members,
                                                                role: member_role,
                                                            }).await;
                                                ...
                                                RaftMessage::TelemetryHeartbeat {
                                                    ...
                                                    let node_id = aaron_core::Uuid::new(node_id_high, node_id_low);
